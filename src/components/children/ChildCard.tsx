    ...
    <StaggerItem>
      <Link href={`/children/${child.id}`}>
        <Card className="p-5 flex flex-col items-center text-center h-full">
          ...
          <p className="text-[12px] text-[var(--text-secondary)]">{cls?.name} · Age {child.age}</p>
          <div className="flex items-center gap-3 mt-3 text-[11px] text-[var(--text-secondary)]">
            ...
            <span className="flex items-center gap-1">
              <Cake size={12} /> {parseDate(child.birthday).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
            </span>
          </div>
          <div className="w-full mt-3.5 pt-3 border-t border-[var(--border-softer)]">
            ...
            <p className="text-[10.5px] text-[var(--text-secondary)] mt-1">{child.attendanceRate}% attendance</p>
          </div>
        </Card>
      </Link>
    </StaggerItem>
