    ...
    <div className="grid lg:grid-cols-2 gap-6">
      <div>
        <h3 className="font-display font-semibold text-lg mb-4 flex items-center gap-2">
          <ShieldAlert size={17} /> {t("admin.moderationHeading")}
        </h3>
        {posts.length === 0 ? (
          <p className="text-sm text-[var(--text-secondary)]">{t("admin.moderationNoPosts")}</p>
        ) : (
                ...
                <StaggerItem key={p.id}>
                  <Card className="p-4">
                    <div className="flex items-center gap-2.5 mb-2">
                      ...
                      <Badge tone="neutral">{enumLabels.postType[language][p.type]}</Badge>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => togglePin(p.id)} className="flex items-center gap-1 text-[11.5px] font-semibold text-[var(--color-gold-deep)]">
                        <Pin size={12} /> {p.pinned ? t("admin.moderationUnpinAction") : t("admin.moderationPinAction")}
                      </button>
      ...
      <div>
        <h3 className="font-display font-semibold text-lg mb-4">{t("admin.moderationAuditLogHeading")}</h3>
        {auditLog.length === 0 ? (
          <p className="text-sm text-[var(--text-secondary)]">{t("admin.moderationNoActivity")}</p>
        ) : (
          <div className="relative pl-5 space-y-4 before:absolute before:left-[5px] before:top-2 before:bottom-2 before:w-px before:bg-[var(--border-soft)]">
            {auditLog.map((entry) => (
              <div key={entry.id} className="relative">
                ...
                {/* Audit log entry text (actor/action/target) is composed server-side in English sentences
                    (see src/app/api/teachers/route.ts etc.) and is out of scope for translation here. */}
                ...
                <p className="text-[11px] text-[var(--text-secondary)]">{timeAgo(entry.date, language)}</p>
              </div>
            ))}
          </div>
        )}
